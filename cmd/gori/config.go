...
import (
	...
	"github.com/gorievm/go-gori/accounts/scwallet"
	"github.com/gorievm/go-gori/accounts/usbwallet"
	"github.com/gorievm/go-gori/beacon/blsync"
...
func loadConfig(file string, cfg *goriConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gori.ipc"
	return cfg
}

// loadBaseConfig loads the goriConfig based on the given command line
// parameters and config file.
func loadBaseConfig(ctx *cli.Context) goriConfig {
	// Load defaults.
	cfg := goriConfig{
...
// makeConfigNode loads gori configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, goriConfig) {
...
// makeFullNode loads gori configuration and creates the Gori backend.
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Create gauge with gori system and build information
	if eth != nil { // The 'eth' backend may be nil in light mode
		var protos []string
		for _, p := range eth.Protocols() {
			protos = append(protos, fmt.Sprintf("%v/%d", p.Name, p.Version))
		}
		metrics.NewRegisteredGaugeInfo("gori/info", nil).Update(metrics.GaugeInfoValue{
			"arch":      runtime.GOARCH,
			"os":        runtime.GOOS,
	...
	// Add the Gori Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *goriConfig) {
