...
func (g *gorirpc) killAndWait() {
	g.gori.Kill()
	g.gori.WaitExit()
}

func (g *gorirpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gori.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gorirpc) addPeer(peer *gorirpc) {
	g.gori.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gori.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gori.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gori.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gori.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gorirpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gorirpc {
	ipcName := fmt.Sprintf("gori-%d.ipc", nextIPC.Add(1))
	...
	ipcpath := ipcEndpoint(ipcName, g.gori.Datadir)
	// We can't know exactly how long gori will take to start, so we try 10
...
func initGeth(t *testing.T) string {
	...
	t.Logf("Initializing gori: %v ", args)
	g := runGeth(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gorirpc {
	datadir := initGeth(t)
	t.Logf("Importing keys to gori")
...
func startClient(t *testing.T, name string) *gorirpc {
	datadir := initGeth(t)
