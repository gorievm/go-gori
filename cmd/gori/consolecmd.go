...
import (
	...
	"github.com/gorievm/go-gori/console"
...
See https://gori.ethereum.org/docs/interacting-with-gori/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gori.ethereum.org/docs/interacting-with-gori/javascript-console.
This command allows to open a console on a running gori node.`,
	}
...
JavaScript API. See https://gori.ethereum.org/docs/interacting-with-gori/javascript-console`,
	}
)

// localConsole starts a new gori node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote gori instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gori: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gori node, attaches an ephemeral JavaScript
