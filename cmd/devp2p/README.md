...
Use `devp2p enrdump <base64>` to verify and display an Gori Node Record.
...
1. initialize the gori node with the `genesis.json` file
2. import blocks from `chain.rlp`
3. run the client using the resulting database. For gori, use a command like the one below:
