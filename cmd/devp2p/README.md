...
Use `devp2p enrdump <base64>` to verify and display an Ori Node Record.
...
1. initialize the gori node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gori with the following flags:
```
gori --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gori node:
 ```
...
To run the eth66 protocol test suite, initialize a gori node as described above and run the following command,
replacing `<enode>` with the enode of the gori node:
...
[dns-tutorial]: https://gori.ethereum.org/docs/developers/gori-developer/dns-discovery-setup
