...
import (
	...
	"github.com/gorievm/go-gori/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	...
	gori, err := runGeth("./testdata", jwtPath)
	if err != nil {
		t.Fatalf("could not run gori: %v", err)
	}
	defer gori.Close()

	suite, err := NewSuite(gori.Server().Self(), "./testdata", gori.HTTPAuthEndpoint(), common.Bytes2Hex(secret[:]))
	if err != nil {
...
// runGeth creates and starts a gori node
