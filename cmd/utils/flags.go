...
import (
	...
	bparams "github.com/gorievm/go-gori/beacon/params"
	"github.com/gorievm/go-gori/common"
	"github.com/gorievm/go-gori/common/fdlimit"
	...
	"github.com/gorievm/go-gori/eth"
	"github.com/gorievm/go-gori/eth/catalyst"
	"github.com/gorievm/go-gori/eth/downloader"
	"github.com/gorievm/go-gori/eth/ethconfig"
	"github.com/gorievm/go-gori/eth/filters"
	"github.com/gorievm/go-gori/eth/gasprice"
	"github.com/gorievm/go-gori/eth/tracers"
	"github.com/gorievm/go-gori/ethdb"
	"github.com/gorievm/go-gori/ethdb/remotedb"
	"github.com/gorievm/go-gori/ethstats"
	"github.com/gorievm/go-gori/graphql"
	"github.com/gorievm/go-gori/internal/ethapi"
	"github.com/gorievm/go-gori/internal/flags"
	"github.com/gorievm/go-gori/log"
	"github.com/gorievm/go-gori/metrics"
	"github.com/gorievm/go-gori/metrics/exp"
	"github.com/gorievm/go-gori/metrics/influxdb"
	...
	"github.com/gorievm/go-gori/triedb"
	"github.com/gorievm/go-gori/triedb/hashdb"
	"github.com/gorievm/go-gori/triedb/pathdb"
...
var (
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the gori attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gori account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Gori client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Gori) {
	backend, err := eth.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Gori service: %v", err)
	}
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
	return backend.APIBackend, backend
}

// RegisterEthStatsService configures the Gori Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Gori Stats service: %v", err)
	}
}
...
func RegisterFullSyncTester(stack *node.Node, eth *eth.Gori, target common.Hash) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gori.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gori.", tagsMap)
		}
...
func DialRPCWithHeaders(endpoint string, headers []string) (*rpc.Client, error) {
	...
	if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gori < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
