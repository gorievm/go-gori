...
`
	...
	{
		gori := runGeth(t, "account", "list", "--datadir", datadir)
		gori.Expect(want)
		gori.ExpectExit()
	}
	{
		gori := runGeth(t, "--datadir", datadir, "account", "list")
		gori.Expect(want)
		gori.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gori := runGeth(t, "account", "new", "--lightkdf")
	defer gori.ExpectExit()
	gori.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gori := runGeth(t, "account", "-h")
	gori.WaitExit()
	if have, want := gori.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gori = runGeth(t, "account", "import", "-h")
	gori.WaitExit()
	if have, want := gori.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gori := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gori.ExpectExit()
	gori.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gori := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gori.ExpectExit()
	gori.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gori.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gori := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gori.StderrText(), m) {
...
// https://github.com/gorievm/go-gori/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gori.SetTemplateFunc("keypath", func(file string) string {
