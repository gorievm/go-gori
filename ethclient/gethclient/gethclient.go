...
// Package goriclient provides an RPC client for gori-specific APIs.
package goriclient
...
// Client is a wrapper around rpc.Client that implements gori-specific functionality.
//
// If you want to use the standardized Gori RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gori node.
...
// MemStats retrieves the current memory stats from a gori node.
...
// GetNodeInfo retrieves the node info of a gori node.
