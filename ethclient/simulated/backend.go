...
// Client exposes the methods provided by the Gori RPC client.
...
// other code that interacts with the Gori chain.
type Backend struct {
	node   *node.Node
...
func NewBackend(alloc types.GenesisAlloc, options ...func(nodeConf *node.Config, ethConf *ethconfig.Config)) *Backend {
	// Create the default configurations for the outer node shell and the Gori
	...
	// Assemble the Gori stack to run the chain with
