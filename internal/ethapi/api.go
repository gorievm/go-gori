...
// EthereumAPI provides an API to access Ori related information.
type EthereumAPI struct {
	b Backend
}

// NewEthereumAPI creates a new Ori protocol API.
...
// Sign calculates an Ori ECDSA signature for:
...
//
// https://github.com/gorievm/go-gori/wiki/Management-APIs#personal_sign
...
//
// https://github.com/gorievm/go-gori/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, errors.New("invalid Ori signature (V is not 27 or 28)")
	}
...
// BlockChainAPI provides an API to access Ori blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Ori blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Ori chain config.
//
...
// DebugAPI is the collection of Ori APIs exposed over the debugging
// namespace.
