...
const ourPath = "github.com/gorievm/go-gori" // Path to our module
...
// conventions in the Gori p2p network.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gori".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gori %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gori %s", params.VersionWithMeta)
	}
