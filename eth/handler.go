...
import (
	...
	"github.com/gorievm/go-gori/core/forkid"
...
// support all the operations needed by the Gori chain protocols.
...
// newHandler returns a handler for all Gori chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Gori handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Gori peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Gori peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Gori peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Gori peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Gori peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Gori protocol stopped")
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	...
	for _, tx := range txs {
		...
		//
		// To do this, we hash the local enode IW with togorier with a peer's
		// enode ID togorier with the transaction sender and broadcast if
