...
import (
	...
	"github.com/gorievm/go-gori/params/forks"
	"github.com/gorievm/go-gori/rpc"
)

// Register adds the engine API to the full node.
func Register(stack *node.Node, backend *eth.Gori) error {
...
func NewConsensusAPI(eth *eth.Gori) *ConsensusAPI {
...
func newConsensusAPIWithoutHeartbeat(eth *eth.Gori) *ConsensusAPI {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gori", ttd, "beacon", config.TerminalTotalDifficulty)
