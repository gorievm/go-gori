...
// data on disk when two subchains are joined togorier.
...
//
// Opposed to the original Gori block synchronization which is trustless (and
...
// not from within (vs. legacy Gori sync).
//
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altogorier
