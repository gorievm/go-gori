...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Ori address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Ori account at a particular block.
    type Account {
    ...
    # Transaction is an Ori transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Ori account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Ori account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Ori block by number or by hash. If neither is
