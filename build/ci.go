...
import (
	...
	"github.com/gorievm/go-gori/crypto/signify"
	"github.com/gorievm/go-gori/internal/build"
	"github.com/gorievm/go-gori/params"
)

var (
	// Files that end up in the gori*.zip archive.
	goriArchiveFiles = []string{
		"COPYING",
		executablePath("gori"),
	}

	// Files that end up in the gori-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Gori contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Gori bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Gori Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gori",
			Description: "Gori CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Gori account management tool.",
		},
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/gorievm/go-gori/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/gorievm/go-gori/internal/version.gitDate="+env.Date)
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "goristore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basegori = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		gori     = "gori-" + basegori + ext
		alltools = "gori-alltools-" + basegori + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gori, goriArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gori, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gori-release.pub"
...
func doDocker(cmdline []string) {
	...
	if *image {
		...
		for _, tag := range tags {
			goriImage := fmt.Sprintf("%s:%s-%s", *upload, tag, runtime.GOARCH)
			...
			// master branch.
			for _, img := range []string{goriImage, toolImage} {
			...
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:TAG", *upload), goriImage)
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:alltools-TAG", *upload), toolImage)
			build.MustRunCommand("docker", "push", goriImage)
	...
	if len(*manifest) != 0 {
		...
		for i := 0; i < 2; i++ { // 2 attempts, second is race check
			...
			for _, tag := range tags {
				for _, arch := range strings.Split(*manifest, ",") {
					goriImage := fmt.Sprintf("%s:%s-%s", *upload, tag, arch)
		...
		for _, tag := range tags {
			goriImage := fmt.Sprintf("%s:%s", *upload, tag)

			var goriSubImages []string
			for _, arch := range strings.Split(*manifest, ",") {
				goriSubImages = append(goriSubImages, goriImage+"-"+arch)
			}
			build.MustRunCommand("docker", append([]string{"manifest", "create", goriImage}, goriSubImages...)...)
			build.MustRunCommand("docker", "manifest", "push", goriImage)
		}
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gori-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gori-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Gori Builds <fjl@ethereum.org>"
	}
	return debMetadata{
		PackageName: name,
		Env:         env,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gori.exe" {
			goriTool = file
		} else {
	...
	// first section contains the gori binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     goriTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gori.nsi", filepath.Join(*workdir, "gori.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gori-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gori.nsi"),
	)
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "goristore/builds")`)
